...
def extract(frame_H, frame_W, cap, rectangle, center_of_gravity, json_dot_data, name):
	...
	# 保存先パス
	path_img1 = "./result2/" + str(name) + "_1"
	path_img2 = "./result2/" + str(name) + "_2"
	...
	while (1):
		...
		try:
			...
			sholder_left = (json_dot_data[frame_count][5 * 3] / 2, json_dot_data[frame_count][5 * 3 + 1] / 2)
			elbow_left = (json_dot_data[frame_count][6 * 3] / 2, json_dot_data[frame_count][6 * 3 + 1] / 2)
			wrist_left = (json_dot_data[frame_count][7 * 3] / 2, json_dot_data[frame_count][7 * 3 + 1] / 2)
			...
			# json_dataに従ってドットを打つ
			# img1, hip 以外
			x = json_dot_data[frame_count][1 * 3]//2
			y = json_dot_data[frame_count][1 * 3 + 1]//2
			...
			for parts in range(2, 5):
				...
				color_img = cv2.circle(color_img, (int(x), int(y)), 1, (0, 255, 255), 3)
			for parts in range(5, 14):
				x = json_dot_data[frame_count][parts * 3]//2
				y = json_dot_data[frame_count][parts * 3 + 1]//2
				color_img = cv2.circle(color_img, (int(x), int(y)), 1, (0, 0, 255), 3)
			...
			# line
			# img
			color_img = cv2.line(color_img, (int(wrist_right[0]), int(wrist_right[1])), (int(elbow_right[0]), int(elbow_right[1])), (0, 255, 255), 1);
			color_img = cv2.line(color_img, (int(sholder_right[0]), int(sholder_right[1])), (int(elbow_right[0]), int(elbow_right[1])), (0, 255, 255), 1);

			color_img = cv2.line(color_img, (int(wrist_left[0]), int(wrist_left[1])), (int(elbow_left[0]), int(elbow_left[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(sholder_left[0]), int(sholder_left[1])), (int(elbow_left[0]), int(elbow_left[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(sholder_right[0]), int(sholder_right[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(sholder_right[0]), int(sholder_right[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(sholder_left[0]), int(sholder_left[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(sholder_left[0]), int(sholder_left[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(hip_right[0]), int(hip_right[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(hip_right[0]), int(hip_right[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(hip_left[0]), int(hip_left[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(hip_left[0]), int(hip_left[1])), (int(neck[0]), int(neck[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(hip_right[0]), int(hip_right[1])), (int(knee_right[0]), int(knee_right[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(ankle_right[0]), int(ankle_right[1])), (int(knee_right[0]), int(knee_right[1])), (0, 0, 255), 1);

			color_img = cv2.line(color_img, (int(hip_left[0]), int(hip_left[1])), (int(knee_left[0]), int(knee_left[1])), (0, 0, 255), 1);
			color_img = cv2.line(color_img, (int(ankle_left[0]), int(ankle_left[1])), (int(knee_left[0]), int(knee_left[1])), (0, 0, 255), 1);
